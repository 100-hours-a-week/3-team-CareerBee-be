...
@Slf4j
public class MonthlySummaryTasklet implements Tasklet {
    ...
    public RepeatStatus execute(StepContribution contrib, ChunkContext ctx) {
        log.info("[{}] MonthlySummaryTasklet start", date);
        summaryService.weekAndMonthSummary(
            DateUtil.getPeriod(date, SummaryType.MONTH), SummaryType.MONTH);
